...
class EventAgent:
    ...
    def find_information(self, conversation, information_extraction):
        ...
        output = self.model.model(new_messages, max_new_tokens=200)
    ...
    def find_event(self, conversation):
        results = self.find_information(conversation,
    ...
    def create_meeting_event(self, conversation):
        try:
            current_date = datetime.now().strftime("%Y-%m-%d")
            current_time = datetime.now().strftime("%H:%M")

            meetings, meetings_time = self.find_information(conversation,
    ...
    def create_milestone_event(self, conversation):
        try:
            current_date = datetime.now().strftime("%Y-%m-%d")

            milestones, milestones_time = self.find_information(conversation,
    ...
    def create_task_event(self, conversation):
        try:
            current_date = datetime.now().strftime("%Y-%m-%d")

            tasks, task_time = self.find_information(conversation,
    ...
    def create_event(self, payload):
        ...
        if event_type == EventType.MEETING:
            results = self.create_meeting_event(conversation)
        elif event_type == EventType.MILESTONE:
            results = self.create_milestone_event(conversation)
        elif event_type == EventType.TASK:
            results = self.create_task_event(conversation)
        else:
            return {
    ...
    def test_agent(self, payload):
        conversations = [
            [
                ...
                {"type": "text",
                 "text": "Alex: Okay, Wednesday afternoon is looking good. What time works for everyone?"},
